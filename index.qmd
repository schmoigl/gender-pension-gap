...
function generateIncomeChart(group) {
  // Filter data
  let filteredIncome = hhIncome.filter(d => d.group == group && d.variable == select);
...
Die Szenarien haben einen Einfluss auf die äquivalenten Haushaltseinkommen und die Armutsquote. Wählen Sie ein Szenario aus um zu sehen welche Gruppen von bestimmten Szenarien am stärksten profitieren.
...
![](img/Infobox_info_icon.svg){height=14px}
Armutsquote: Die relative Armutsquote misst den Anteil der Menschen in einer Gesellschaft, deren Einkommen unterhalb einer bestimmten Grenze liegt. Diese Grenze wird in der Regel bei 60 % des mittleren (Median-) Einkommens der Bevölkerung festgelegt. Menschen, die weniger als diesen Betrag verdienen, gelten als relativ arm, da sie im Vergleich zum Durchschnitt deutlich weniger Mittel zur Verfügung haben.
:::
...
viewof haushaltseffekt = Inputs.radio(["Haushaltseinkommen", "Armutsquote"], {label: "Effekte auf", value: "Haushaltseinkommen"})
```
...
```{ojs}
{
  ...
  generateIncomeChart("Geschlecht") : generatePovertyChart("Geschlecht");
}
```
...
{
  return haushaltseffekt === "Haushaltseinkommen" ?
  generateIncomeChartStatusquo("Geschlecht") : generatePovertyChartStatusquo("Geschlecht");
}
```
...
```{ojs}
Inputs.bind(Inputs.select(scenarios, {label: "Szenario"}), viewof select)
Inputs.bind(Inputs.radio(["Haushaltseinkommen", "Armutsquote"], {value: "Haushaltseinkommen", label: "Effekte auf"}), viewof haushaltseffekt) 
```
