...
<div class="image-container"> <img src="img/WIFO_6340.jpg"> </div>
...
:::{.infoText}
¹Datenbasis: Pensionsversicherungs-Jahresstatistik (PJ), Dezember 2023 Berechnung: MA 23 - Wirtschaft, Arbeit und Statistik der Stadt Wien. <br>
²Datenbasis: Basierend auf Sozialversicherungsdaten und den Erwerbs- und Einkommensverläufen werden für die Pensionsantrittskohorten 2015 bis 2021.. 
:::
...
interpolate = "step"
check1Info = select != "Szenario 1 (Kindererziehung)" ? "Kindererziehungszeiten sind nur im Szenario 1 relevant." : radio != "Frauen" ? "Kindererziehungszeiten wurden bei der Berechnung der Szenarien nur für Frauen berücksichtigt." : "Kindererziehungszeiten werden für Frauen in durchschnittlichem Ausmaß angenommen."
check2Info = select != "Szenario 2 (Arbeitslosigkeit)" ? "Arbeitslosenversicherungsszeiten sind nur im Szenario 2 relevant." : "Es werden Arbeitslosenversicherungszeiten von durchschnittlicher Höhe angenommen."
```
...
::: {.slide}
#### Ergebnisse 1/4
<span>Wie würde sich das Szenario auf vergleichbare Personen auswirken?</span>
...
Inputs.bind(
  Inputs.radio(
    ...
    {value: "Vergleichsgruppe", label: "Zeige"}
    ),
  viewof radio2
  )
```
...
barsText = 
  ...
  "Im " + select + " würden vergleichbere " + radio + " mit einer Pension von " + d3.format(".0f")(statusIncome) + " Euro (" + deciles.map(d => d.closestDecileValue)[0] + " der " + radio + ")" + checkText1 + checkText2 + " im Schnitt um " + d3.format(".0f")(diffScenario) + " Euro mehr Pension erhalten. Das entspricht etwa " + d3.format(".0%")(diffScenarioPerc) + " mehr. Betrachtet man den Durchschnitt aller "+ radio + checkText1 + checkText2 + " so würden diese im Schnitt " + d3.format(".0f")(diffScenarioGesamt) + " Euro oder " + d3.format(".0%")(diffScenarioPercGesamt) + " mehr Pension bekommen." :
  diffScenarioGesamt < 0 ?
  "Im " + select + " würden vergleichbere " + radio + " mit einer Pension von " + d3.format(".0f")(statusIncome) + " Euro (" + deciles.map(d => d.closestDecileValue)[0] + " der " + radio + ")" + checkText1 + checkText2 + " im Schnitt um " + d3.format(".0f")(diffScenario) + " Euro mehr Pension erhalten. Das entspricht etwa " + d3.format(".0%")(diffScenarioPerc) + " mehr. Betrachtet man den Durchschnitt aller "+ radio + checkText1 + checkText2 + " so würden diese im Schnitt " + d3.format(".0f")(-diffScenarioGesamt) + " Euro oder " + d3.format(".0%")(-diffScenarioPercGesamt) + " weniger Pension bekommen." : 
  "Im " + select + " würden " + radio + " mit einer Pension von " + d3.format(".0f")(statusIncome) + " Euro" + checkText2 + " gleich viel erhalten wie aktuell. " + "Das Szenario hätte keinen Effekt. " 
```
...
Inputs.bind(Inputs.radio(["Frauen", "Männer"], {value: "Frauen", label: "Geschlecht"}), viewof radio)
...
::: {.slide}
#### Ergebnisse 2/4
<span>Wie würde sich die Änderung auf den Gender Pension Gap auswirken?</span>
...
dumbbell(radio2 == "Beides anzeigen" ? decilesDumbbellFull : decilesDumbbellFull.filter(d => d.D == radio2), width > 600 ? 230 : width * 0.38).render({ renderer: "svg" }) 
...
::: {.slide}
#### Ergebnisse 4/4 
<span>Wie würden sich die Verteilungen dieser Pensionen im Szenario ändern?</span>
...
distributionsChange = {
  ...
  const colorsArea = radio == "Frauen" || select == "Szenario 2 (Arbeitslosigkeit)" || select == "Szenario 5 (Beste 15 Jahre)"  ? colors : [colors[0], "transparent"]
  ...
  const textGrayVar = radio == "Frauen" || select == "Szenario 2 (Arbeitslosigkeit)" || select == "Szenario 5 (Beste 15 Jahre)"  ? "neue mittlere Pension " + radio : "mittlere Pension " + radio 
  ...
  const textNewVar = radio == "Frauen" || select == "Szenario 2 (Arbeitslosigkeit)" || select == "Szenario 5 (Beste 15 Jahre)"  ? "⤺ neue Pensionen" : "⤺ keine Veränderung"
...
<div class="fold-container">
  ...
  <div class="column">
      ...
      ```{ojs}
      Inputs.bind(Inputs.radio(["Vergleichsgruppe", "∅ Durchschnitt"], {value: "Vergleichsgruppe", label: "Zeige"}), viewof radio2)
...
::: {.column width="50%"}
```{ojs}
bars(width > 600 ? 250 : width * 0.66).render({ renderer: "svg" })
```
:::

::: {.column width="50%"}
```{ojs}
dumbbell(radio2 == "Beides anzeigen" ? decilesDumbbellFull.filter(d => d.D == "Vergleichsgruppe") : decilesDumbbellFull.filter(d => d.D == radio2), width > 600 ? 230 : width * 0.66).render({ renderer: "svg" })
```
:::
