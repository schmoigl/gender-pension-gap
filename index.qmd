...
// guess // is null
answer = guess == "40,1 %" ? "Richtig! Der Unterschied ist groß." : "Leider falsch, die Realität sieht anders aus."
```
...
::: {.slide}
**Ergebnisse 1/4**: 
Wie würde sich das Szenario auf vergleichbare Personen auswirken?
```{ojs}
Inputs.bind(
  Inputs.radio(
    ["Vergleichsgruppe", "∅ Durchschnitt", "Beides anzeigen"], 
...
dumbbell(radio2 == "Beides anzeigen" ? decilesDumbbellFull : decilesDumbbellFull.filter(d => d.D == radio2)).render({ renderer: "svg" }) 
```
```{ojs}
...
::: {.column width="50%"}
```{ojs}
dumbbell(radio2 == "Beides anzeigen" ? decilesDumbbellFull.filter(d => d.D == "Vergleichsgruppe") : decilesDumbbellFull.filter(d => d.D == radio2)).render({ renderer: "svg" })
```
:::
...
<div class="btn-group" role="group" aria-label="Social Media Share Buttons">
  <!-- Teilen: &nbsp; -->
  <a href="https://twitter.com/intent/tweet?text=Schaut%20euch%20dase%20an&url=https://data-science.wifo.ac.at/AK-Pensionsreformrechner /" target="_blank" class="btn btn-secondary">
  ...
  <a href="mailto:?subject=Schau%20dir%20das%20an&body=https://data-science.wifo.ac.at/AK-Pensionsreformrechner /" target="_blank" class="btn btn-secondary">
