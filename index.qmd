...
textInteractive2 = radio == "Marie" ?
  html`Würden Sie ${notRadio == "Otto" ? "ein" : "eine"} <strong><span style="color: ${otherGenderColor}">${notRadio}</span></strong> sein und im <strong>${closestDecileValue}</strong>, dann würden Sie <strong>${germanLocale.format(",.0f")(otherGenderIncome)}</strong> Euro an Pension bekommen. Das sind <strong>${germanLocale.format(",.0f")(gapAbsolute)}</strong> Euro ${gapDirection} (<strong>${germanLocale.format(",.0f")(firstPercent)}%</strong>). Oder anders herum formuliert: <strong><span style="color: ${colors2[0]}">Marie</span></strong> hätte <strong>${germanLocale.format(",.0f")(secondPercent)}%</strong> weniger als <strong><span style="color: ${colors2[1]}">Otto</span></strong>. So groß ist also der Gender Pension Gap im <strong>${closestDecileValue}</strong>. Im <strong>Median</strong> beträgt er <strong>47%</strong> oder <strong>1.063</strong> Euro.`
  :
  html`Würden Sie ${notRadio == "Otto" ? "ein" : "eine"} <strong><span style="color: ${otherGenderColor}">${notRadio}</span></strong> sein und im <strong>${closestDecileValue}</strong>, dann würden Sie <strong>${germanLocale.format(",.0f")(otherGenderIncome)}</strong> Euro an Pension bekommen. Das sind <strong>${germanLocale.format(",.0f")(gapAbsolute)}</strong> Euro ${gapDirection} (<strong>${germanLocale.format(",.0f")(firstPercent)}%</strong>). So groß ist also der Gender Pension Gap im <strong>${closestDecileValue}</strong>. Im <strong>Median</strong> beträgt er <strong>47%</strong> oder <strong>1.063</strong> Euro.`
```
...
Vor hundert Jahren revolutionierten <strong><span style="color:#be0021">Marie</span></strong> und <strong><span style="color:#ffa500">Otto</span></strong> Neurath am Wiener Gesellschafts- und Wirtschaftsmuseum die Darstellung komplexer sozialer und ökonomischer Zusammenhänge. Mit ihrer Bildstatistik haben sie Verteilungsfragen sichtbar und verständlich gemacht. 

Heute, 2025, zeigen die Daten: Der Gender Pension Gap besteht weiterhin. <strong><span style="color:#be0021">Marie</span></strong> erhält in Österreich deutlich weniger Pension als <strong><span style="color:#ffa500">Otto</span></strong>. Die nachfolgenden interaktiven Visualisierung ermöglicht es, diese Pensionsunterschiede nach Geschlecht zu erkunden. In der Tradition der Neuraths wird komplexe Information zugänglich gemacht, um gesellschaftliche Probleme sichtbar zu machen und darüber hinaus textlich beschrieben.

::: {.column-margin}
Die Daten basieren auf der Studie von <strong>Eppel et al.</strong> [-@eppel2024simulation] zur Simulation von Änderungen des Pensionssystems auf die Höhe der Alterseinkommen und wurden zunächst auch im Rahmen des Folgeprojekts <strong>Mayrhuber et al.</strong> [-@wifo2024pension] entwickelt.
:::
...
```{ojs}
viewof rangeRaw = Inputs.range([200, 4000], {step: 50, value: 1500, label: "Bruttopension:"})
