...
## Frauenpensionen erhöhen: AK-Vorschläge für eine faire Berechnung
...
<button class="btn btn-secondary" style="margin-top: 30px; margin-bottom: 50px;" onclick="jumpToSlide(1)">Weiter und herausfinden, wie sich Ihre Pension verändern würde</button>
...
<button class="btn btn-secondary" style="margin-top: 30px; margin-bottom: 50px;" onclick="jumpToSlide(11)">Weiter und herausfinden, wie sich Armutsgefährdungsquote und Einkommen verändern</button>
:::
...
Mit dem AK-Pensionsfairnessrechner sehen Sie, was die einzelnen Reformen für die Pensionshöhen, bei der Reduktion des Gender Pension Gap und Verteilung bedeuten. Probieren Sie das neue Tool aus, verbreiten Sie es und geben Sie uns Rückmeldung!
...
Die Auswirkungen dieser Szenarien können Sie hier erkunden. Der Rechner zeigt Ihnen dabei, wie sich die Pensionshöhe in den jeweiligen Szenarien für Personen mit Ihren Merkmalen durchschnittlich verändern würde.
...
dumbbellText = check1.length != 1 && select == "Szenario 1 (Höherbewertung Kindererziehungszeiten)" && radio == "Frauen" ?
  "Der Gender Pension Gap in meiner Vergleichsgruppe würde sich nicht verändern. Er beträgt " + germanLocale.format(",.0f")(deciles.filter(d => d.D == d.closestDecileValue && d.val == "Status quo").map(d => d.gapValue)[0]) + " oder " + germanLocale.format(",.0%")(deciles.filter(d => d.D == d.closestDecileValue && d.val == "Szenario").map(d => d.gapRel)[0]) + " relativ zur Pension der Männer. Insgesamt würde der Gap von " + germanLocale.format(",.0f")(deciles.filter(d => d.D == "Gesamt" && d.val == "Status quo").map(d => d.gapValue)[0]) + " Euro auf " + germanLocale.format(",.0f")(deciles.filter(d => d.D == "Gesamt" && d.val == "Szenario").map(d => d.gapValue)[0]) + " Euro" + mehrWenigerGesamt + " Das entspricht dann " + germanLocale.format(",.0%")(deciles.filter(d => d.D == "Gesamt" && d.val == "Szenario").map(d => d.gapRel)[0]) + " der Männerpension." :
  check2.length != 1 && select == "Szenario 2 (Höherbewertung Zeiten der Arbeitslosigkeit)" ?
  "Der Gender Pension Gap in meiner Vergleichsgruppe würde sich nicht verändern. Er beträgt " + germanLocale.format(",.0f")(deciles.filter(d => d.D == d.closestDecileValue && d.val == "Status quo").map(d => d.gapValue)[0]) + " oder " + germanLocale.format(",.0%")(deciles.filter(d => d.D == d.closestDecileValue && d.val == "Szenario").map(d => d.gapRel)[0]) + " relativ zur Pension der Männer. Insgesamt würde der Gap von " + germanLocale.format(",.0f")(deciles.filter(d => d.D == "Gesamt" && d.val == "Status quo").map(d => d.gapValue)[0]) + " Euro auf " + germanLocale.format(",.0f")(deciles.filter(d => d.D == "Gesamt" && d.val == "Szenario").map(d => d.gapValue)[0]) + " Euro" + mehrWenigerGesamt + " Das entspricht dann " + germanLocale.format(",.0%")(deciles.filter(d => d.D == "Gesamt" && d.val == "Szenario").map(d => d.gapRel)[0]) + " der Männerpension." :
...
Probieren Sie das neue Tool aus, verbreiten Sie es und geben Sie uns gerne Rückmeldung!
...
Die Auswirkungen dieser Szenarien können Sie hier erkunden. Der AK-Pensionsfairness-Rechner zeigt Ihnen dabei, wie sich die Haushaltseinkommen und die Armutsgefährdungsquote in den jeweiligen Szenarien für Personen mit Ihren Merkmalen durchschnittlich verändern würden. 
...
#### Auswahl 1/2
```{ojs}
viewof radioHaushaltseffekt = Inputs.radio(["das Gesamteinkommen meines Haushalts", "die Armutsgefährdungsquote meines Haushalts"], {value: "das Gesamteinkommen meines Haushalts"})

optionHH = radioHaushaltseffekt === "das Gesamteinkommen meines Haushalts" ? 
  html`Haushaltseinkommen` : html`Armutsgefährdungsquote`

infoHH = radioHaushaltseffekt === "das Gesamteinkommen meines Haushalts" ? 
  html`
...
```{ojs}
html`
${radioHaushaltseffekt === "das Gesamteinkommen meines Haushalts"
...
hhIncomeGeschlechttext = 
  ...
  radioRaw != "Frau" && select == "Szenario 1 (Höherbewertung Kindererziehungszeiten)" ?
   "Das durchschnittliche äquivalisierte Haushaltseinkommen von Männern liegt mit 2.528€ leicht über dem der Frauen (2.470€)¹. Sie würden von Szenario 1 weniger profitieren (+0,26%) als Frauen (+0,47%)." : 
...
${radioHaushaltseffekt === "das Gesamteinkommen meines Haushalts" ? hhIncomeGeschlechttext : hhPovertyGeschlechttext} <br>

```{ojs}
html`${radioHaushaltseffekt === "das Gesamteinkommen meines Haushalts" ?
...
Inputs.bind(Inputs.radio(["Frau", "Mann"], {value: "Frau", label: "Geschlecht"}), viewof radioRaw)
Inputs.bind(Inputs.select(scenarios, {label: "Szenario"}), viewof select)
Inputs.bind(Inputs.radio(["das Gesamteinkommen meines Haushalts", "die Armutsgefährdungsquote meines Haushalts"], {value: "das Gesamteinkommen meines Haushalts", label: "Effekte auf"}), viewof radioHaushaltseffekt) 
```
...
```{ojs}
optionHH2 = radioHaushaltseffekt === "das Gesamteinkommen meines Haushalts" ? 
  html`Haushaltseinkommen` : html`Armutsgefährdungsquote`
...
${radioHaushaltseffekt === "das Gesamteinkommen meines Haushalts" ? hhIncomeAltersgruppetext : hhPovertyAltersgruppetext} <br><br>
...
```{ojs}
optionHH3 = radioHaushaltseffekt === "das Gesamteinkommen meines Haushalts" ? 
  html`Haushaltseinkommen` : html`Armutsgefährdungsquote`
...
${radioHaushaltseffekt === "das Gesamteinkommen meines Haushalts" ? hhIncomeStatustext : hhPovertyStatustext} <br><br>
