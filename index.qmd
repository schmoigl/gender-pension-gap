...
hhIncome = aq.from(hhIncomeRaw)
  ...
  .derive({ 
    ...
    group2: d => d.group != "Haushaltstyp - detailiert" ? "irrelevant" : (d.detail.includes("kein Kind") ? "kein Kind" : "ein Kind"),
    })
  .objects()
...
function generateIncomeChart(group, highlight) {
  ...
  let filteredIncome = hhIncome.filter(d => d.group == group && d.variable == select); // Ensure 'select' is correctly defined or passed in
...
//    vl.stroke(detail == highlight ? vl.value('black') : vl.value('transparent'))
  ...
  return vl 
    ...
    .width(group == "Geschlecht" ? makeWidth(width) * 0.3 : makeWidth(width) * 0.6) // Dynamically set width based on group
...
function generateIncomeChartStatusquo(group) {
  // Filter data
  let filteredIncomeStatusquo = hhIncome.filter(d => d.group == group && d.variable == "Status quo");
...
function generatePovertyChart(group) {
  let filteredPoverty = hhPoverty.filter(d => d.group == group && d.variable == select);
  ...
  return vl 
    ...
    .width(group == "Geschlecht" ? makeWidth(width) * 0.3: makeWidth(width) * 0.6)
...
function generatePovertyChartStatusquo(group) {
  // Filter data
  let filteredPovertyStatusquo = hhPoverty.filter(d => d.group == group && d.variable == "Status quo");
...
```{ojs}
viewof radioKinder = Inputs.radio(["Kein Kind", "Ein Kind", "Zwei Kinder", "Drei Kinder oder mehr"], {label: "Kinder", value: "Kein Kind"})
kinder = radioKinder
```
:::
...
viewof haushaltseffekt = Inputs.radio(["Haushaltseinkommen", "Armutsgefährdungsquote"], {label: "Effekte auf", value: "Haushaltseinkommen"})
```
...
html`
  <div>
    <h3>${haushaltseffekt === "Haushaltseinkommen" ? "Haushaltseinkommen" : "Armutsgefährdungsquote"}</h3>
    ${
      haushaltseffekt === "Haushaltseinkommen" ? 
      html`
        <div class="infoText">
          <img src="img/Infobox_info_icon.svg" height="14px">
...
```{ojs}
{
  return haushaltseffekt === "Haushaltseinkommen" ?
  generateIncomeChart("Geschlecht", "highlight") : generatePovertyChart("Geschlecht");
}
```
...
{
  return haushaltseffekt === "Haushaltseinkommen" ?
  generateIncomeChartStatusquo("Geschlecht", highlight) : generatePovertyChartStatusquo("Geschlecht");
}
...
html`
  <div>
    <span>${haushaltseffekt === "Haushaltseinkommen" ? hhIncomeGeschlechttext : hhPovertyGeschlechttext}</span>
  </div>
`
html`<div style="height: 20px;"></div>`
```
...
```{ojs}
Inputs.bind(Inputs.select(scenarios, {label: "Szenario"}), viewof select)
Inputs.bind(Inputs.radio(["Haushaltseinkommen", "Armutsgefährdungsquote"], {value: "Haushaltseinkommen", label: "Effekte auf"}), viewof haushaltseffekt) 
```
...
{
  return haushaltseffekt === "Haushaltseinkommen" ?
  generateIncomeChart("Altersgruppen") : generatePovertyChart("Altersgruppen");
}
```
...
{
  return haushaltseffekt === "Haushaltseinkommen" ?
  generateIncomeChartStatusquo("Altersgruppen") : generatePovertyChartStatusquo("Altersgruppen");
}
html`<div style="height: 20px;"></div>`
```
...
html`
  <div>
    <span>${haushaltseffekt === "Haushaltseinkommen" ? hhIncomeAltersgruppetext : hhPovertyAltersgruppetext}</span>
  </div>
`
html`<div style="height: 20px;"></div>`
```
...
{
  return haushaltseffekt === "Haushaltseinkommen" ?
  generateIncomeChart("Erwerbsstatus") : generatePovertyChart("Erwerbsstatus");
}
```
...
{
  return haushaltseffekt === "Haushaltseinkommen" ?
  generateIncomeChartStatusquo("Erwerbsstatus") : generatePovertyChartStatusquo("Erwerbsstatus");
}
html`<div style="height: 20px;"></div>`
```
...
html`
  <div>
    <span>${haushaltseffekt === "Haushaltseinkommen" ? hhIncomeStatustext : hhPovertyStatustext}</span>
  </div>
`
html`<div style="height: 20px;"></div>`
```
...
html`
  <div>
    <h2>${radioKinder === "Kein Kind" ? "Haushalte ohne Kinder" : "Haushalte mit Kindern"}</h3>
  </div>
`
```
...
html`
  <div>
    <h3>${haushaltseffekt === "Haushaltseinkommen" ? "Haushaltseinkommen" : "Armutsquote"}</h3>
  </div>
`
```
...
{
  return haushaltseffekt === "Haushaltseinkommen" ?
  generateIncomeChart("Haushaltstyp - detailiert") : generatePovertyChart("Haushaltstyp - detailiert");
}
```
...
{
  return haushaltseffekt === "Haushaltseinkommen" ?
  generateIncomeChartStatusquo("Haushaltstyp - detailiert") : generatePovertyChartStatusquo("Haushaltstyp - detailiert");
}
```
...
<script>
  ...
  function updateButtonStates() {
    ...
    if (currentSlide < 2) {
      ...
      jumpToLastBtn.style.display = 'inline-block'; // 'inline-block'
    } else {
