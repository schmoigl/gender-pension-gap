...
textInteractive2 = radio == "Marie" ?
  ...
  :
  html`Würden Sie ${notRadio == "Otto" ? "ein" : "eine"} <strong><span style="color: ${otherGenderColor}">${notRadio}</span></strong> sein und im <strong>${closestDecileValue}</strong>, dann würden Sie <strong>${germanLocale.format(",.0f")(otherGenderIncome)}</strong> Euro an Pension bekommen. Das sind <strong>${germanLocale.format(",.0f")(gapAbsolute)}</strong> Euro ${gapDirection} (<strong>${germanLocale.format(",.0f")(firstPercent)}%</strong>).`
```
